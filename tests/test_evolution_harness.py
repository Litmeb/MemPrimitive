...
from memprimitive.evolution._codex import codex_exec_args, run_orchestrator, run_worker_codex
from memprimitive.evolution._process import CommandRunner, ProcessResult
...
def test_codex_exec_args_include_sandbox_and_output_file(tmp_path: Path) -> None:
    ...
    args = codex_exec_args(config=config, cwd=tmp_path, sandbox="read-only", output_last_message=output)
