"""Demo: successful-trial feedback keeps the Reflexion-style trigger closed.
...
Or from this directory (script adds the repo root to ``sys.path``)::
    ...
    python reflexion_trigger_success_trial.py
"""
...
# Running as ``python memprimitive/example/demonstration/reflexion_trigger_success_trial.py``
...
def main() -> None:
    ...
    packet = pipeline.ingest(
        Observation(
            text="Trial scratchpad: parser handles the edge case correctly.",
            source="dialogue",
            metadata={
                "is_correct": True,
                "feedback": "The answer matches the expected output.",
                "trial_index": 3,
            },
        )
    )
