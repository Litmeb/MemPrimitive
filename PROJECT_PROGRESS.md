...
- The benchmark CLI can also run the classics MemMachine memory-layer reconstruction on LoCoMo via `--memory-adapter memmachine`. Mem0 and MemMachine now expose the shared benchmark memory binding surface (`build_system`, `ingest_event`, `recall`), but their LoCoMo wiring intentionally differs: Mem0 still uses `DualSpeakerLoCoMoMemoryAdapter` with two speaker-view systems, while MemMachine now uses one shared conversation memory, ingests each raw message individually, immediately indexes each message into LTM/sentence layers, recalls with `limit=30, expand_context=3`, and renders timestamped XML-like memory context through template readout plus a MemMachine-specific LoCoMo answer prompt. New systems that implement `create_memory_binding()` can also be loaded through `--memory-adapter binding --memory-binding module:create_memory_binding`.
...
- Runtime rerank now uses an independent OpenAI-compatible `/rerank` API configured by `MEMPRIMITIVE_RERANK_API_KEY`, `MEMPRIMITIVE_RERANK_BASE_URL`, and `MEMPRIMITIVE_RERANK_MODEL`; `Runtime.rerank()` no longer falls back to an LLM JSON prompt.
...
- `memprimitive/example/classics/memmachine_memory.py` now provides a compact executable memory-layer reconstruction: append raw episodes into working memory, maintain profile features through `LLMFunctionCallEvolution` + profile feature tools, immediately index every episode into LTM plus parent-linked sentence derivatives, use STM consolidation only for working-memory overflow summary/trimming, and recall with timestamped template readout. The LoCoMo adapter uses one shared conversation memory with per-message ingest and a MemMachine-specific answer prompt; it intentionally excludes the optional Retrieval Agent router.
- LoCoMo audit follow-up after fixing `benchmarks/outputs/locomo_memmachine_gpt4omini_user1*.json*` root causes: the benchmark wiring no longer uses the old dual-speaker pairwise path that prevented LTM population under `stm_record_budget=20`. The new MemMachine LoCoMo path writes each raw message to one shared conversation memory, immediately populates `episodic` and `sentence`, recalls with official-style `limit=30, expand_context=3`, returns non-empty source ids, and renders `<LONG TERM MEMORY EPISODES>` / `<WORKING MEMORY SUMMARY>` timestamped sections.
