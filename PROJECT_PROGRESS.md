...
- A first store-targeted hierarchical abstraction family has now been added on top of that selector path: `HierarchicalOrganization` and `HierarchicalEvolution` can consume `packet.decisions_store`, group selected source-layer records, and append higher-level target-layer records with metadata-native provenance edges under `metadata["hierarchical"]`. This is the first baseline family that intentionally treats `decisions_store` as an execution-time store selector rather than only a trigger trace artifact.
