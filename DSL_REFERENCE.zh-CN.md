...
| `GraphSeedAndExpandRetrieval` (`graph_seed_and_expand_retrieval`) | `top_k=3`, `layer="knowledge_graph"`, `seed_top_k=2`, `include_seed_records=True` | 先选 graph seed，再做一跳邻居扩展。 |
| `ExpandRetrievedGraphNeighbors` (`expand_retrieved_graph_neighbors`) | `neighbor_limit=3`, `include_seed_records=True` 等 | 以当前 `packet.retrieved.items` 为种子继续扩图。 |
